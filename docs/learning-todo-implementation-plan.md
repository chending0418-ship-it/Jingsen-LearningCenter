...
- 家长可为任务设置自定义 Reward 目标和点数；任务完成后由家长确认发放，重复确认不会重复加分。
- 每个重复任务实例独立记录 Reward，发放时固化目标、点数和确认时间；该部分从任务 JSON 直接汇总。

总积分计算为 `连续完成积分 + 已由家长确认的任务 Reward 积分`。孩子端积分卡片展示总积分、两个来源小计、当前连续完成任务日数和今天已获得或可争取分数。积分只读取 `data/learning-todo/tasks/*.json`，不会读写词库、Skills 或 Daily Reports。
...
| GET | `/api/todo/reward` | 总积分、连续完成积分、任务 Reward 积分及今日积分状态 |
...
任务管理接口另提供 `POST /api/admin/todo/tasks/{task_id}/reward/confirm`，仅允许在任务完成且配置了 Reward 后确认；接口为幂等发放。
