...
## 2026-09-17：英语词库合并与原词库状态处理（已上线）

- 目的：将已有英语词库汇总为新的复习词库，同时让管理员决定原词库继续保留、停用或归档。
- Admin → Learning Center 的「词库操作」新增「合并英语词库」，页面地址为 `/admin/learningcenter/merge`。支持搜索、多选至少两个未归档英语词库（含已停用词库）、新名称、创建后是否启用、逐个或批量设置原词库处理方式。
- 合并去除首尾空格、忽略大小写去重，保留首次出现的写法和词条顺序；预览显示原词条总数、重复数、合并后数量、完整词条及原词库处理清单。保留现状不改变原启用状态，停用和归档均停止参与出题；原词库内容完整保留，归档后仍可恢复。
- 新增受 Admin 会话保护的 `POST /api/admin/libraries/merge-preview` 与 `/merge`，同时支持 `/learningcenter` 前缀。预览不写数据；校验新名称、来源存在性、学科、归档状态、重复选择和空内容。预览版本绑定来源内容、状态和合并设置，过期时要求重新预览。
- 使用单个 SQLite `BEGIN IMMEDIATE` 事务创建新词库、写入词条和更新所选来源状态，失败全部回滚，并防止并发重复提交创建同名词库。仅更新所选词库，保留其他词库、词条记录和练习历史；无需新增依赖或修改数据库结构。
- 验证：隔离数据库全量测试 `106 passed`（新增 28 项合并测试），覆盖三种处理方式、新词库启用/停用、去重与顺序、预览无写入、失效预览、无效请求、归档恢复、重启读取、事务失败回滚、并发提交、权限与两套 API 路径。浏览器完整验证 3 个来源的保留/停用/归档混合操作、9 条合并为 5 条、原内容保留、详情入口、搜索保留选择、修改设置后预览失效及 390px 手机布局；无页面脚本错误。JavaScript 语法和 Git 空白检查通过。
- 边界：已归档词库需先取消归档再参与合并；去重按词条文本处理，不合并不同拼写或不同释义。功能验证使用独立测试数据，正式词库不会自动合并；上线继续采用完整数据快照和安全发布流程。
- 发布：功能提交 `2908730` 已推送到 `codex/english-library-merge` 和生产分支 `deploy/tencent-learningcenter-path`，经 `update_safe.sh` 上线。发布期间暂停服务以冻结写入，完成备份及恢复后重启；完整快照保留在 `/www/wwwroot/learningcenter/backups/releases/20260917-163044-928416`。
- 发布后核对：27 张数据库表逻辑内容与快照一致，保留 49 个词库、3,477 条词汇、635 项 Skills、214 条英语报告、1 本书/24 个章节/1 次阅读/4 道阅读问题，以及 1,328 条 Todo 与 3,378 条历史；140 个非数据库文件和 `.env` 内容一致，SQLite 完整性及外键检查通过。
- 公网和内部健康、English、Math、后台共用样式均返回 200；新合并页面未登录返回 303，两个合并 API 未登录返回 401；有效 Admin 会话下首页入口、新页面和两套路径的只读合并预览均验证通过。未在正式数据中创建测试词库或执行实际合并。

## 2026-09-17：Math 因式分解训练与独立后台记录（已上线）

### 变更目的与用户可见结果

- 将 Math 建设中入口替换为可使用的因式分解专项，沿用当前网站的米白底、蓝色、荧光黄、黑色细线、直角布局以及 Manrope / DM Mono 字体体系，兼容桌面和手机。
- 每轮固定 10 题：2 道公因式、2 道基本公式、2 道连续平方差、2 道多方法综合、1 道换元和 1 道添项配方思考题。覆盖 `x⁴ − 16` 连续分解，以及补上再减去 `1` 或含字母项的配方；思考题在首次展示时不泄露解题方法。
- 新增点击式数学键盘：数字、x、二/三/四次方、自动配对括号、整体平方、退格、撤销和移动光标。可选草稿提供 t 与等号；草稿保存但不参与最终答案判分。
- 每题首次提交后保存并展示结果与解析；区分答错、等价但未分解彻底、跳过和正确。语法错误允许修正后再提交；提示分级展示，独立答对与提示后答对分开记录。支持暂停、刷新恢复和本轮错题回看。
- 在 Admin → Learning Center 新增 Math 入口 `/admin/learningcenter/math`，可分页查看已完成及进行中的训练、完成时间、正确率、独立答对数、原题、首次答案、参考答案、错误反馈、解析、草稿和提示次数，并筛选错题。
- 按用户明确要求，Math 历史独立管理，不接入英语 Daily Reports，不修改英语历史接口、页面或记录。

### 实现与数据

- 规则构造题目，使用受限的整数多项式解析器及 SymPy 检查等价与完整分解；不调用 AI，也不通过 eval / sympify / parse_expr 执行学生输入。限制表达式长度、括号深度、次数和系数大小。
- SQLite Schema v5 仅新增 `math_sessions` 与 `math_session_questions`，保留原有数据；完成第 10 题时在同一事务内更新结果。使用 `BEGIN IMMEDIATE` 防止跨 worker 重复计分，相同提交幂等返回，不允许覆盖已保存的首次答案。
- 孩子端通过随机凭证恢复训练，凭证仅以哈希存储；未答题目的参考答案和解析不下发。历史页面与 API 沿用现有 Admin 会话保护。兼容根路径与 `/learningcenter` 前缀。
- 新增依赖 `sympy>=1.13,<2`。更新 README 与 SQLite 字段字典。开发分支为 `codex/math-factorization`。

### 验证与后续边界

- 全量自动测试 `78 passed`，含新增 30 项数学专项用例；覆盖 500 道生成题、等价写法、完整性判题、受限输入、10 题固定计划、权限、首次答案不可覆盖、并发最终提交、提示幂等、恢复、后台分页、增量迁移及英语报告不变。
- 真实 Chrome 完整点击完成一轮 10 题，刻意制造错误、未分解彻底和跳过，验证结果页及 Admin 均为预期的 70% 正确率、6/10 独立答对、3 道错题/未完成。检查草稿与刷新恢复、提示、后台过滤、桌面/手机页面，无页面 JavaScript 错误。
- JavaScript 语法与 Git 空白检查通过；所有接口、浏览器及迁移验证使用隔离的测试数据库，未用于线上数据。现有 FastAPI lifespan 弃用提示不属于本次变更。
- 功能提交 `46c221d` 已推送到 `codex/math-factorization` 和生产分支 `deploy/tencent-learningcenter-path`，通过 `update_safe.sh` 发布；服务器安装 SymPy 1.14.0，SQLite 迁移 1–5 均已登记。发布期间短暂停止服务，避免快照与恢复期间产生新写入；服务恢复后保持 `enabled`、`active`。
- 发布前完整快照保留在 `/www/wwwroot/learningcenter/backups/releases/20260917-131247-879879`。发布后核对全部旧业务表逻辑内容一致，包括 214 条英语报告、49 个词库及 3,477 个词条、635 项 Skills、1 本书/24 个章节/1 次阅读/4 道阅读问题、1,327 条 Todo 及 3,376 条历史；139 个原有非数据库文件和 `.env` 内容一致，SQLite 完整性正常且无外键违规。
- 公网 Math 页面及四个资源、English、Reading、健康接口均返回 HTTP 200；Math Admin 未登录时页面/API 分别返回 303/401，带有效管理会话时页面与两个 API 路径均返回 200。服务器实际运行时使用临时隔离数据库完成一轮 10 题及完整/未分解彻底判分校验，未在生产历史写入测试训练；发布后近十分钟服务日志无 Error、Traceback 或 Exception。
- 首版范围为整数系数、单变量 x 的因式分解；草稿仅保存，暂不逐行评分。后续发布继续使用完整数据快照及安全更新流程。
