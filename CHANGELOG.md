...
- 孩子可以键盘输入，也可以用浏览器录音后转成可编辑文字。原始录音仅用于该次转写，不保存到服务器。
...
- 新增根路径和 `/learningcenter` 前缀兼容的 Reading API；语音转写模型默认使用 `gpt-transcribe`，文件大小及模型名称可由环境变量调整。

### 验证

- 全量自动测试通过：`44 passed`，新增覆盖 Admin 会话保护、上传/识别/发布、公开书架、随机凭证、问题与追问、文字/语音答案、完成评估、家长报告、聚焦目录请求和兼容接口 JSON 代码块解析。
...
- 扫描图片型 PDF 尚未接 OCR/Vision；音频转写还依赖当前 API 服务支持 `gpt-transcribe`。正式发布前应使用实际书籍、实际录音和生产所选模型做一次费用、延迟与兼容性验收。
- 当前配置的 OpenAI 兼容服务已验证可完成目录识别、问题生成和回答评估，但其 `/audio/transcriptions` 接口返回 HTTP 403；线上文字阅读流程可用，语音输入需在供应商开放转写接口或配置独立转写服务后才能使用。
